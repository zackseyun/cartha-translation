...
## Drafting process (Phase 2 — Azure GPT-5.4-mini + review layer)
...
- Deployment: `gpt-5-4-mini-ko`
- Model label: `gpt-5.4-mini`

Important: do **not** continue Korean drafting on `gpt-5-mini-atlas`. The
script is pinned to GPT-5.4-mini by default and refuses non-GPT-5.4-mini
deployments unless `CARTHA_KO_ALLOW_NON_54_MINI=1` is intentionally set for a
one-off comparison. As of the 2026-05-22 Azure check, this account has a small
`gpt-5-4-mini-ko` Data Zone Standard deployment; broad drafting should wait for
the submitted GPT-5.4-mini quota increase or run at very low concurrency.
