...
def git_head() -> tuple[str, str]:
    # status.json is generated by automation. If we point the public
    # status payload at the status commit itself, the hourly job creates
    # a self-sustaining loop: every generated status commit changes the
    # next generated status payload. Anchor it to the latest content
    # commit instead.
    try:
        full = subprocess.check_output(
            ["git", "rev-list", "-1", "HEAD", "--", ".", ":!status.json"],
            cwd=REPO_ROOT,
            text=True,
        ).strip()
    ...
    short = subprocess.check_output(
        ["git", "rev-parse", "--short", full], cwd=REPO_ROOT, text=True
    ).strip()
    return full, short


def comparable_status(payload: dict) -> dict:
    comparable = dict(payload)
    comparable.pop("generated_at", None)
    return comparable


def preserve_generated_at_if_unchanged(payload: dict) -> dict:
    ...
    try:
        ...
        return payload
    if comparable_status(previous) == comparable_status(payload):
...
def main() -> int:
    ...
    payload = preserve_generated_at_if_unchanged(payload)
