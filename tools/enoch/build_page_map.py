#!/usr/bin/env python3
"""build_page_map.py — assemble page_map.json from chapter_detection caches.

Consumes `sources/enoch/ethiopic/chapter_detection/<edition>.json` and
proposes a `page_map.json` covering every chapter detected. Writes to
`page_map.proposed.json` so the maintainer can diff before promoting.

Rules:
  - chapter_start with chapters [a, b, c]: each owns the page; the
    LAST chapter on the page also owns subsequent continuation pages.
...
PROPOSED = REPO_ROOT / "sources" / "enoch" / "ethiopic" / "page_map.proposed.json"
...
def chapter_hint(n: int) -> str:
    return f"chapter {n} (ምዕራፍ {geez_numeral(n)})"
    ...
    for page in sorted(classifications.keys()):
        ...
        if kind == "chapter_start":
            ...
            if not chapters:
                ...
                continue
            for c in chapters:
                chapter_pages.setdefault(c, [])
                if page not in chapter_pages[c]:
                    chapter_pages[c].append(page)
            first = chapters[0]
            if first < last_seen_chapter:
                # Out-of-order chapter — could be model error (numeral confusion).
                warnings.append(f"{edition} p{page}: chapter {first} starts after later chapter {last_seen_chapter} was seen earlier — likely numeral misread")
            else:
...
def load_existing_map() -> dict[str, Any]:
    if not PAGE_MAP.exists():
        return {"book": "1_enoch", "updated": dt.datetime.now(dt.timezone.utc).strftime("%Y-%m-%d"), "editions": {}}
    return json.loads(PAGE_MAP.read_text(encoding="utf-8"))


def build_edition_section(pdf_rel: str, chapter_pages: dict[int, list[int]], existing: dict[str, Any]) -> dict[str, Any]:
    existing_chapters = existing.get("chapters", {})
    out: dict[str, Any] = {}
    for chapter in sorted(chapter_pages.keys()):
        key = f"{chapter:03d}"
        prior = existing_chapters.get(key, {})
        out[key] = {
            "pages": chapter_pages[chapter],
            "chapter_hint": prior.get("chapter_hint") or chapter_hint(chapter),
            "opening_hint": prior.get("opening_hint", ""),
        }
    return {
        "pdf": pdf_rel,
        "notes": existing.get("notes", "Page map proposed by tools/enoch/build_page_map.py."),
        "chapters": out,
    }
...
def summarise(edition: str, proposed: dict[str, Any], existing: dict[str, Any]) -> str:
    ...
    for key in sorted(set(new_chapters) & set(existing_chapters)):
        if existing_chapters[key]["pages"] != new_chapters[key]["pages"]:
            changed.append((key, existing_chapters[key]["pages"], new_chapters[key]["pages"]))
    ...
    missing = sorted(c for c in range(1, 109) if f"{c:03d}" not in new_chapters)
    if missing:
        head = ", ".join(str(c) for c in missing[:30])
        lines.append(f"  STILL MISSING ({len(missing)}): {head}{' …' if len(missing) > 30 else ''}")
    else:
        lines.append("  all 108 chapters covered ✓")
    return "\n".join(lines)
...
def main() -> int:
    ...
    for edition in args.editions:
        classifications = load_detection(edition)
        chapter_pages, warnings = assemble_chapters(classifications, edition=edition)
        existing_section = existing.get("editions", {}).get(edition, {})
        pdf_rel = existing_section.get("pdf", f"sources/enoch/scans/{edition}_ethiopic.pdf")
        proposed.setdefault("editions", {})[edition] = build_edition_section(pdf_rel, chapter_pages, existing_section)
    ...
    print()
    for edition in args.editions:
        print(summarise(edition, proposed["editions"][edition], existing.get("editions", {}).get(edition, {})))
        print()
    if all_warnings:
